...
public class RemovalFilterTest extends TestCase
{
	...
	String s3 = "foo";
	String s4 = "";
	ReplacementFilter f = new RemovalFilter("[0-9]", Location.START);
	...
	String s3 = "foo";
	String s4 = "";
	ReplacementFilter f = new RemovalFilter("[0-9]", Location.END);
	...
	String s3 = "foo";
	String s4 = "";
	ReplacementFilter f = new RemovalFilter("[0-2]", Location.ALL);
	
	assertEquals("abcdefg44", f.filter(s0, true));
	assertEquals("abc34de34fg", f.filter(s1, true));
	assertEquals("abcd34efg34", f.filter(s2, true));
	assertEquals("foo", f.filter(s3, true));
	assertEquals("", f.filter(s4, true));
	
	assertEquals("abcdefg44", f.filter(s0, false));
	assertEquals("abc34de34fg", f.filter(s1, false));
	assertEquals("abcd34efg34", f.filter(s2, false));
	assertEquals("foo", f.filter(s3, false));
	assertEquals("", f.filter(s4, false));
    }
